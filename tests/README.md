...
Criterion uses `[profile.bench]` (LTO, single codegen unit). Fixtures: `json0`, `canada`, `twitter`, and `citm_catalog` under `benches/data/` (see [`benches/data/README.md`](../benches/data/README.md)). For flamegraphs, build with `--profile profiling` (or `profile.samply`).
